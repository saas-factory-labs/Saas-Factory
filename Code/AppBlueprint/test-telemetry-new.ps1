...
# Check if OpenTelemetry packages are properly referenced
Write-Host "`nVerifying Package References:" -ForegroundColor Cyan
Write-Host "--------------------------------" -ForegroundColor Gray
...
if (Test-Path $packagesPath) {
    ...
    foreach ($pkg in $otelPackages) {
        if ($packages -match "$pkg.*Version=""([^""]+)""") {
            Write-Host "$pkg version: $($Matches[1])" -ForegroundColor Green
        } else {
