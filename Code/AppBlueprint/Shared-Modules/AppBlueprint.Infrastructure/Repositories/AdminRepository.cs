...
public class AdminRepository : IAdminRepository
{
    ...
    {
        await _context.Set<AdminEntity>().AddAsync(admin);
    }

    public void Update(AdminEntity admin)
    {
        _context.Set<AdminEntity>().Update(admin);
    }

    public void Delete(int id)
    {
        AdminEntity? admin = _context.Set<AdminEntity>().Find(id);
        if (admin is not null) _context.Set<AdminEntity>().Remove(admin);
    }
}
