...
public partial class AddSignupStoredProcedure : Migration
{
    ...
    {
        // Read the stored procedure SQL from the CreateSignupStoredProcedure.sql file
...
BEGIN
    ...
    IF LENGTH(TRIM(p_user_first_name)) < 1 OR LENGTH(TRIM(p_user_last_name)) < 1 THEN
        RAISE EXCEPTION 'First name and last name are required';
    END IF;
    
    -- Validate tenant name
    IF LENGTH(TRIM(p_tenant_name)) < 1 THEN
        RAISE EXCEPTION 'Tenant name is required';
    END IF;
    
    -- ========================================
    -- STEP 3: Rate Limiting (Check for abuse)
    -- ========================================
    -- Allow max 5 signups per email per hour
...
GRANT EXECUTE ON FUNCTION create_tenant_and_user TO PUBLIC;
...
";
        ...
        // Execute the stored procedure SQL
    ...
    {
        // Drop the stored procedure and related objects in reverse order
        migrationBuilder.Sql(@"
            -- Drop functions
            DROP FUNCTION IF EXISTS create_tenant_and_user(TEXT, TEXT, INTEGER, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT);
            DROP FUNCTION IF EXISTS email_exists(TEXT);
            DROP FUNCTION IF EXISTS validate_email_format(TEXT);
            DROP FUNCTION IF EXISTS validate_id_format(TEXT, TEXT);
            
            -- Drop audit table and indexes
            DROP INDEX IF EXISTS idx_signup_audit_ip_created;
            DROP INDEX IF EXISTS idx_signup_audit_email_created;
