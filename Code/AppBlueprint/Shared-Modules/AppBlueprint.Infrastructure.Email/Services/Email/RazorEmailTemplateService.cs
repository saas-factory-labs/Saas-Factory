...
public sealed class RazorEmailTemplateService : IEmailTemplateService
{
    ...
    {
        var assembly = typeof(RazorEmailTemplateService).Assembly;
        IReadOnlyList<string> templates = assembly.GetManifestResourceNames()
            .Where(name => name.EndsWith(".cshtml", StringComparison.OrdinalIgnoreCase))
            .Select(name =>
            {
                string fileName = name.Split('.').SkipLast(1).Last();
                return fileName;
            })
        ...
        CancellationToken cancellationToken = default) where TModel : IEmailTemplateModel
    {
        ...
        if (_resend is null)
        {
            const string errorMessage = "Cannot send email: Resend is not configured.";
        ...
        {
            _logger.LogError(ex, "Network error sending templated email {TemplateName} to {Recipient}", templateName, to);
            throw;
        }
        catch (TaskCanceledException ex)
        {
            _logger.LogError(ex, "Timeout sending templated email {TemplateName} to {Recipient}", templateName, to);
            throw;
        }
        catch (RazorLightException ex)
        {
            _logger.LogError(ex, "Template rendering failed for {TemplateName}", templateName);
            throw;
        }
    }
}
